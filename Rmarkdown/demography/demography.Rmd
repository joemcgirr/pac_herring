...
# Summary
This notebook generates scripts for demographic inference in Pacific herring populations using unlinked biallelic SNPs and the [moments pipeline](https://github.com/dportik/moments_pipeline).

These analyses are exploratory. The 1D and 2D sfs look strange even without a minor allele frequency threshold. It's likely because low coverage data leads to an overestimate of singletons, doubletons, etc. I experimented with several different ways of generating 1D and 2D sfs.
...
`ph_filtered_snps_minDP600_maxDP2000_minQ20_minMQ30_NS0.5.vcf` available [here](https://osf.io/3cmkr/)
...
#projections 
proj = 82
dd = moments.Misc.make_data_dict('C:/Users/jmcgirr/Documents/Whitehead_Lab/ph/moments/downsample/chr1_5mb/'+pops[0] + '_' + pops[1]+'_dadi_snp.data')
fs = moments.Spectrum.from_data_dict(dd,pop_ids=[pops[0], pops[1]],projections=[proj, proj],polarized = True)
S = fs.S()
Fst = fs.Fst()
print(S)
print(Fst)
...
Results (migration rates, time at split, Ne) are inconsistent. 
...
# ms     <-c()
# ts     <-c()
# for (param_row in c(1:nrow(params))){
# opt_params <- strsplit(params[1,8], ",")[[param_row]]
# 
# theta <- c(thetas ,as.numeric(params$theta[param_row]))
# nu1   <- c(nu1s   ,as.numeric(opt_params[1]))
# nu2   <- c(nu2s   ,as.numeric(opt_params[2]))
# m     <- c(ms     ,as.numeric(opt_params[3]))
# t     <- c(ts     ,as.numeric(opt_params[4]))
# 
# }
