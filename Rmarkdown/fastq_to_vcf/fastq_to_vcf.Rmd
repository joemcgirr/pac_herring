...
# Summary
This notebook is used to generate multiple `.vcf` files containing variant data for 892 Pacific herring samples (low coverage whole genomes).

Data info: 
- `fastq`, `bam`, and `.vcf` files are currently located on the Farm sever at UC Davis (/home/eoziolor/phpopg/data/align/).
- Raw `g.vcfs` were generated from `.bam` files aligned to the Atlantic herring reference genome [c. harengus Ch_v2.0.2 on Ensembl](https://uswest.ensembl.org/Clupea_harengus/Info/Index?db=core).
- `.vcf` files are also available [here](https://osf.io/3cmkr/) 
...
`.vcf` is generated for first and second half of each chromosome to avoid maxmimum memory limits.
...
## Create `.vcf` with biallelic SNPs and filter by depth and quality
...
### Generate bcftools filter scripts
```{python,results='hide', eval = FALSE}
...
information about filtering high coverage regions
...
# Concatenate chromosomes into one `.vcf`
use bcftools concat
...
### Generate bcftools concat script
```{python,results='hide', eval = FALSE}
job = 'merge_snp_vcfs'
...
# Intersect filtered vcfs to find common SNPs  
...
### Generate bcftools isec scripts
```{python,results='hide', eval = FALSE}
job_name = 'isec'
...
# Filter final vcf to include SNPs with minor allele frequency > 0.05
...
## Plot depth and number of samples genotyped (DP and NS)
```{r, message=FALSE, warning=FALSE, fig.width= 10, fig.height= 7}
